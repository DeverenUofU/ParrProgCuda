...
__global__ void mmtt(const float *A, const float *B, float *C, int ds) {  //ds should be 1024 I beleive
...
//   h_Cref[i][j] += h_A[k][i]*h_B[j][k];
 ...
 // Allocate device memory and copy input data over to GPU
  ...
  dim3 block(256,1);  
  //dim3 grid(4,4);
  //dim3 grid((DSIZE + block.x - 1) / block.x, 1);  // Adjust grid to cover DSIZE
  //dim3 grid((DSIZE * DSIZE + block.x - 1) / block.x, 1);  // Total threads >= DSIZE^2
  //dim3 grid((DSIZE + block.x - 1) / block.x, (DSIZE + block.y - 1) / block.y);  // Enough blocks to cover matrix
  dim3 grid((DSIZE + block.x - 1) / block.x, DSIZE);  
