...
__global__ void mmtt_j4(const float *A, const float *B, float *C, int ds) {
    ...
    if (i < ds && j < ds) {  ///does this whole thing need to be adjusted to be the i version?
        float sum0 = 0.0;
        float sum1 = 0.0;
        float sum2 = 0.0;
        float sum3 = 0.0;
        for (int k = 0; k < ds; ++k) {
            ...
            sum1 += A[i + ds*k] * B[k + ds*(j+1)];
            sum2 += A[i + ds*k] * B[k + ds*(j+2)];
            sum3 += A[i + ds*k] * B[k + ds*(j+3)];
        }
        C[i * ds + j] = sum0;
        C[i * ds + (j+1)] = sum1;
        C[i * ds + (j+2)] = sum2;
...
//   dim3 grid((DSIZE + block.x - 1) / block.x, DSIZE);  
  dim3 grid( ((DSIZE + block.x - 1)) / block.x, DSIZE / 4);  // Adjusted for y dim,  j-loop unrolling
