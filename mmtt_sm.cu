...
__global__ void mmtt_sm(const float *A, const float *B, float *C, int ds) {
    ...
    int tx = threadIdx.x;
    int i = blockIdx.x * blockDim.x + tx;  //when x changes, i changes
    int j = blockIdx.y;                 // j does not change with x
    _shared_ float bbuf[256];

    if (i < ds && j < ds) {
        float sum = 0.0;
        for (int ks = 0; k < ds; ks += blockDim.x) 
        {
            bbuf[tx] = B[j*ds + ks + tx] //or somethiung
            _syncthreads();
            for (int k = ks; k < min(ds,ks+blockDim.x); k++) 
            {
                sum += bbuf[k - ks] * A[k*ds + i]  //    A[k*ds + i] * B[k + ds*j];
            }
            _syncthreads();
        }
        C[i * ds + j] = sum;
